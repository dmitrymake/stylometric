# Stylo — воспроизводимая атрибуция авторства русской прозы
...
> [+0.0377, +0.1938],
> P(Δ≤0)=0.001. Из-за upstream cross-work
...
признаков дал 0.8805, а мешок слов — 0.8008; McNemar
**p = 0.00018** сохранён только для воспроизведения и не является действующим
свидетельством значимости. Cosine Delta 0.7888
и legacy selected-mass Delta по целым книгам 0.7371 также
...
нециркулярный leave-block-out LOBO — атрибутирует **3/4 тома ТД → Шолохову**
...
- **Историческая калибровка**: ECE = 0.2078 — это ПЛОХАЯ калибровка
...
**Исторический PD-only бенчмарк (22 автора-классика (†70+ лет) / 159 книг; snapshot ineligible):**
...
| **ансамбль (равновесный, historical)** | **0.931** | **0.913** |
...
**Историческая диагностика каналов:** на полном бенчмарке (43 автора) равновесный ансамбль каналов под
*другим* классификатором (LinearSVC + StratifiedGroupKFold(5), а не LR+LOBO) даёт macro-F1 0.833 /
top-1 0.892 (`docs/validation.json`). Это старый diagnostic view, не
