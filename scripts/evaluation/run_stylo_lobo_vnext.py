#!/usr/bin/env python3
"""Run the fail-closed real-corpus LOBO-vNext control plane.
...
the historical A0/A1/A4 runner.  Real-corpus operation has two separate
explicit modes: an authorization-free, fit-free execution-spec preflight and
an exact interactive-authorization-bound bounded exploratory dry run.
"""
...
def _requested_mode(argv: Sequence[str]) -> str:
    if "--synthetic-dry-run" in argv:
        raise VNextCLIError(
            "--synthetic-dry-run was removed; the canonical LOBO-vNext "
            "runner is real-only"
        )
    selected = [
    ...
    if len(selected) != 1:
        raise VNextCLIError(
            "exactly one real execution mode must be selected: "
            "--real-preflight or --real-exploratory-dry-run"
        )
    return selected[0]
