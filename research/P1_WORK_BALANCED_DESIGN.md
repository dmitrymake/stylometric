# P1 — work-balanced core: design v3 (for Codex final spot-check)

Status: design v3 + six-point contract patch, 2026-07-13. Author: Claude Opus.
Reviewer: **Codex**. **Phase A is signed GO** (242/242 suite, 15/15 target tests, 1000
random ragged corpora verified, no P0 drift). This revision applies the six narrow contract
refinements from the last audit (no architecture change): B0 fail-closed only (§1) + manifest
validator (§1); B1 sparse numerical edges (§3); B2 no Stacked double-weighting (§4);
B3 effective calibration folds (§5); B4 completed artifact contract (§6). Nothing in phases
B–E is implemented. All of B–E lives behind `evaluation.training_weighting` (default
`chunk_weighted_legacy`) so P0 is byte-reproducible until the paired audit (Phase E) signs off.

## 0. Phase A — fixed and ready to sign

`src/stylo/eval/work_weighting.py`:

- `sw_i = W / (A·W_a·C_w)`, `sum = W` (D3), strict length checks + `zip(strict=True)`,
  multi-author work rejected.
- **Fix 1:** `resolve_training_weighting` now validates the chosen value *including the
  fallback default* (an invalid default raises). Negative test added.
- **Fix 2:** runtime label ≠ public claim string is made explicit with
  `to_claim_label()`: `chunk_weighted_legacy` → `chunk_weighted_training_legacy` (the P0
  headline claim, unchanged), `work_balanced` → `work_balanced`. The committed P0 artifact
  is not touched.
...
## 1. Canonical work/chunk identity (finding #1) — NOT content hash
...
**Prerequisite (plan §4.1):** spans/provenance are not persisted today —
`chunking.make_sent_chunks` (`chunking.py:18`) and `pipeline/split.py:72` write chunk text
only. Phase B-0 adds a **chunk manifest** (`data/frags_train/<author>/<book>/manifest.json`,
strict JSON) recording per-chunk `span_ordinal`, `text_sha256`, and the work's
`provenance_sha256` + `chunker_config_hash`. `corpus.load_dataset` reads it to build the
canonical `WorkDocument` view over the existing `texts/reps/groups` arrays (no new heavy
format).
...
## 2. Full Pipeline groups contract (finding #2)
...
`pipeline/train.py:45` (which is currently broken because `Pipeline.fit(groups=)` raises).

## 3. Work-level sparse vectorizer — fit AND transform (finding #3)

One shared helper `features/work_vectorizer.py` (fit + transform fully specified):

Fit (per §4 of v2): chunk counts → work×feature sum → work-DF (prune `df_w < 2`) →
...
- function_words: relative frequencies (D2), both modes; denominator = all word tokens.
...
## 4. Five-model wiring spec (finding #4)
...
## 5. Group-aware calibration eligibility (finding, decision 3)

Cloneable group-calibration wrapper builds explicit StratifiedGroupKFold (train,val) work
splits inside the outer-train fold (`eval/calibration.py:84` currently splits OOF by chunk).
...
## 6. Phase E — normalized paired-audit artifact (finding #5)
...
## 9. Implementation order (once signed)

B-0 chunk manifest + canonical WorkDocument + dedup (§1) — the gate for strict duplication.
B-1 `work_vectorizer` fit+transform (§3) + block contract (§2), behind toggle, legacy default.
B-2 five-model wiring (§4) + shared `fit_estimator` dispatch (fixes broken train.py:45).
B-3 group-aware calibration (§5).
B-4 paired audit runner + artifact (§6) with the five-row ladder (§7).
Headline stays untouched until B-4 artifact exists and Codex signs the result.
