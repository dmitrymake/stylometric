...
class StackClassCoverageError(ValueError):
    ...
    ``report`` is intentionally attached to the exception so callers can persist the
    exact failed split inventory without parsing the human-readable message.
    """
...
def build_inner_split_preflight_report(
    ...
    """Pure inventory of class coverage and structure in every supplied split.

    The helper performs no fitting and never short-circuits: every split is reported,
    including every missing class and structural violation.  That makes the preflight
    independently testable and prevents a late channel-specific failure from hiding
    a second bad split.
    """
    labels = np.asarray(y)
    ...
    if expected.ndim != 1 or len(expected) == 0:
        raise ValueError(f"expected_classes must be a non-empty 1-D array, got {expected.shape}")
    ...
    for split_index, (train_index, validation_index) in enumerate(splits):
        ...
        split_reports.append({
            ...
            "observed_train_classes": [_plain_scalar(value) for value in observed],
            "missing_train_classes": [_plain_scalar(value) for value in missing],
            "unexpected_train_classes": [_plain_scalar(value) for value in unexpected],
...
class StackedChannelClassifier:
    ...
    @staticmethod
    def _fit_learner(learner, X, y, *, sample_weight, groups, evidence_role):
        """Fit first, then record the exact weight vector consumed by this successful learner fit."""
        fitted = learner.fit(X, y, sample_weight=sample_weight)
        if sample_weight is not None:
            from ..eval.fold_evidence import record_ordered_weights
            record_ordered_weights(
                sample_weight, groups, role=f"consumed:{evidence_role}"
            )
        return fitted
    ...
    def _decision_full(self, clf, X, n_classes: int, class_map: np.ndarray) -> np.ndarray:
        ...
        if len(unique_map) != len(class_map) or len(missing) or len(unexpected):
            report = {
                ...
                "unexpected_classes": [_plain_scalar(value) for value in unexpected],
    ...
    def fit(self, texts, y, groups=None):
        ...
        # class-coverage preflight.  A bad split therefore cannot trigger even one
        # expensive channel fit (or a partial evidence trace).
        channels = self._channels()

        oof: Dict[str, np.ndarray] = {}
        for name, fn in channels.items():
            # NaN is a fail-closed construction marker, never a model score.  It
            # catches a malformed splitter that omits a validation row without
            # resurrecting the historical numeric sentinel.
            scores = np.full((len(y), n_cls), np.nan, dtype=np.float64)
            for split_index, (tr_i, te_i) in enumerate(splits):
                ...
                weights = self._fold_weights(y[tr_i], groups[tr_i])
                clf = self._fit_learner(
                    self._svc(), Xtr, y[tr_i], sample_weight=weights,
                    groups=groups[tr_i], evidence_role=f"inner_svc:{name}:{split_index}",
                )
        ...
        if calibration_disabled:
            ...
            for split_index, (tr_i, te_i) in enumerate(meta_splits):
                meta = LogisticRegression(C=self.meta_c, max_iter=2000,
                                          class_weight=self._class_weight(), random_state=self.seed)
                weights = self._fold_weights(y[tr_i], groups[tr_i])
                meta = self._fit_learner(
                    meta, Moof[tr_i], y_local[tr_i], sample_weight=weights,
                    groups=groups[tr_i], evidence_role=f"meta_cv:{split_index}",
                )
            ...
            if self.mode_ == "stacked":
                weights = self._fold_weights(y, groups)
                self.meta_ = self._fit_learner(LogisticRegression(
                    C=self.meta_c, max_iter=2000, class_weight=self._class_weight(),
                    random_state=self.seed), Moof, y_local, sample_weight=weights,
                    groups=groups, evidence_role="meta_full")
    ...
    def predict_proba(self, texts) -> np.ndarray:
        ...
        for name in self._channel_names:
            ...
            clf = self._fit_learner(
                self._svc(), Xtr, self._train_y, sample_weight=sw,
                groups=self._train_groups, evidence_role=f"predict_svc:{name}",
            )
