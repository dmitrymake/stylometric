...
def make_factory_for_ablation(spec: str, cfg, *, ablation,
    ...
    * **A1** (weights-only == ``(T,F,F)``) is an audit-only path: the work-balanced loss/calibration
      protocol on a strictly legacy (A0) feature side, wired ONLY for the axis-aware
      estimators (``stylo``/``bow_lr``/``stylo_stack``). A1 has NO production
      weighting enum (``to_weighting`` stays corner-only), so it is never collapsed
      to ``WORK_BALANCED``.
...
def make_factory(spec: str, cfg, enabled_override: Optional[Dict[str, bool]] = None,
    ...
    if spec == "stylo_equal_channels_v1":
