"""Resumable 47-class true-LOBO validation for the signed B4 stylo cells.

This module is deliberately narrower than the future confirmatory B4 control plane.  It evaluates
...
from .b4_pilot import artifact_self_hash, canonical_hash
...
RUN_SCHEMA_VERSION = "b4_true_lobo_run_v1"
GATE_SCHEMA_VERSION = "b4_true_lobo_a0_gate_v1"
RUNTIME_SCHEMA_VERSION = "b4_true_lobo_runtime_v1"
...
def build_run_identity(
    *,
    ...
    body = {
        ...
        "representation_cache": copy.deepcopy(representation_cache),
    }
    body["run_id"] = canonical_hash(body)
    body["self_hash"] = artifact_self_hash(body)
    return body


def validate_run_identity(identity: dict[str, Any]) -> None:
    if not isinstance(identity, dict) or identity.get("schema_version") != RUN_SCHEMA_VERSION:
    ...
    if identity.get("run_id") != canonical_hash(without_hashes):
...
class CheckpointStore:
    ...
    def __init__(self, root: str | pathlib.Path, identity: dict[str, Any]):
        ...
        if run_path.exists():
            ...
            dump_strict(identity, run_path, indent=2, ensure_ascii=False)
    ...
    def save(self, checkpoint: dict[str, Any]) -> pathlib.Path:
        ...
        if path.exists():
            ...
            return path
        dump_strict(checkpoint, path, indent=2, ensure_ascii=False)
        return path
    ...
    def add_runtime(self, cell: str, seconds: float, n_jobs: int) -> None:
        ...
        value["per_cell_execution_wall_seconds"][cell] = float(
            value["per_cell_execution_wall_seconds"][cell] + max(0.0, float(seconds)))
        value["self_hash"] = artifact_self_hash(value)
        dump_strict(value, self.root / "RUNTIME.json", indent=2, ensure_ascii=False)
    ...
    def save_gate(self, receipt: dict[str, Any]) -> None:
        ...
        if path.exists():
            ...
            return
        dump_strict(receipt, path, indent=2, ensure_ascii=False)
...
def assemble_cell(
    ...
    metric_labels = [int(item["label"]) for item in identity["metric_label_order"]]
    per_author = {}
    ...
    record = {
        ...
        "metrics": {
            ...
            "macro_f1": macro_f1(truth, pred, metric_labels),
    ...
    *,
    ...
    if n_jobs == 1:
        results: Iterable[dict[str, Any]] = (
            evaluator(cfg, dataset, cell, fold) for fold in missing
        )
    else:
        results = Parallel(
            n_jobs=n_jobs,
            ...
            delayed(evaluator)(cfg, dataset, cell, fold) for fold in missing
        )
...
def run_true_lobo(
    ...
    *,
    ...
    if type(n_jobs) is not int or n_jobs <= 0:
        raise TrueLoboError("n_jobs must be a positive exact integer")
