# Work-balanced authorship evaluation

Current status has one authority:
[`../governance/status_ledger.json`](../governance/status_ledger.json).

- [`estimand.md`](estimand.md) — the single compact implementation contract.
- [`paired_audit_protocol.md`](paired_audit_protocol.md) — the normative scientific protocol.
- [`paired_audit_implementation_audit.md`](paired_audit_implementation_audit.md) and
  [`paired_audit_review_provenance.md`](paired_audit_review_provenance.md) — an immutable historical
  implementation-audit and review-provenance evidence chain.

Retained historical summaries:
[`exploratory_ablation_screen.md`](exploratory_ablation_screen.md) and
[`stylo_lobo_validation_protocol.md`](stylo_lobo_validation_protocol.md).
