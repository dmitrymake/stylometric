...
| SCI-FINAL-004 | HIGH | fixed | Final exact-commit review showed that wheel verification and Python import resolution were still separate authorities: a same-named package earlier on `sys.path`, or an unrecorded native `__init__` extension beside the verified source, could be imported while the genuine distribution's RECORD identity was recorded. The model loader now binds the standard source-package spec, `__init__.py` origin, single package search root and loaded module back to a SHA-256/size-verified member of the same distribution before and after `spacy.load`. Package-root inventory rejects every unrecorded file or symlink except bytecode proven derivable from a verified source. Primary and fallback shadow-package plus native-extension regressions fail before import; real lg/md primary and fallback loads pass. Frozen evidence, scientific approvals, owner dispositions and confirmatory gates remain unchanged. |
