"""B4-B increment 1: AblationConfig + make_factory routing, WITHOUT changing estimator math.

Only the two corners are runnable (legacy == all-off, work_balanced == all-on); intermediate
single-axis configs raise until later increments wire them. The A0/A4 corners reached through the new
ablation path must reproduce the frozen B4-B1 goldens quantized-exact at the fixture's declared
round_decimals (proved here for the spaCy-free models).
"""
...
def test_make_factory_weighting_contract_unchanged():
    # increment 1 must not weaken the make_factory contract: weighting stays a required keyword
    import inspect
...
def test_goldens_reproduced_via_ablation_path(spec):
    ...
    for corner_name, ablation in (("A0", LEGACY_ABLATION), ("A4", FULL_WB_ABLATION)):
        ...
        np.testing.assert_array_equal(got, want)               # increment 1 must not move the goldens
