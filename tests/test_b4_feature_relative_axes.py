"""B4-B increment 3: the FunctionWord F×R grid and the A2/A3 feature-state estimators.
...
def test_fw_grid_vocab_transform_and_denominator():
    b0, X0, v0 = _fw(None, None)           # A0 F0R0
    b2, X2, v2 = _fw(False, FW_GROUPS)     # A2 F1R0
    b3, X3, v3 = _fw(True, None)           # A3 F0R1
    b4, X4, v4 = _fw(None, FW_GROUPS)      # A4 F1R1
    # F axis: A0==A3 pooled, A2==A4 work, pooled != work; independent pooled reference
    assert v0 == v3 and v2 == v4 and set(v0) != set(v2)
    assert sorted(v0) == sorted(CountVectorizer(max_features=3, lowercase=True, token_pattern=_TP)
                                .fit(FW_TEXTS).get_feature_names_out())
    assert "x" in set(v0) and "x" not in set(v2) and "c" in set(v2) and "c" not in set(v0)
    # R axis: A0/A2 raw integers; A3/A4 fractions == independent selected/all-events; sparse (no densify)
    for X in (X0, X2, X3, X4):
        assert issparse(X)
    assert X0.dtype == np.int64                                                     # A0 raw CSR stays int64
    D0, D2, D3, D4 = (X.toarray() for X in (X0, X2, X3, X4))
    assert np.array_equal(D0, np.round(D0)) and np.array_equal(D2, np.round(D2))    # raw counts
    ev = analyzer_event_counts(_ANALYZER, FW_TEXTS)
    ref3 = _sel_counts(FW_TEXTS, v3) / np.where(ev == 0, 1.0, ev)[:, None]           # independent R1
    ref4 = _sel_counts(FW_TEXTS, v4) / np.where(ev == 0, 1.0, ev)[:, None]
    np.testing.assert_allclose(D3, ref3, atol=1e-12)
    np.testing.assert_allclose(D4, ref4, atol=1e-12)
    assert not np.array_equal(D3, np.round(D3))                                      # genuinely fractional
    assert (b0.feature_fit_, b0.relative_fw_) == (False, False)
    assert (b2.feature_fit_, b2.relative_fw_) == (True, False)
    assert (b3.feature_fit_, b3.relative_fw_) == (False, True)
    assert (b4.feature_fit_, b4.relative_fw_) == (True, True)
...
def test_fw_old_pickle_is_honest_and_stripped_current_artifact_is_rejected():
    fields = ("relative_fw", "relative_fw_", "feature_fit_")
    # a GENUINE pre-inc3 A4 pickle removes the fields from BOTH the top-level state AND _ctor_state; it
    # must still predict relative (MODE_RELATIVE _wv). A genuine pre-inc3 A0 predicts raw.
    ...
    # a CURRENT (inc-3) artifact whose fields are stripped ONLY at the top level (still in _ctor_state)
    # is a corrupt new artifact, NOT an old one: it must be rejected, not downgraded to legacy.
...
@requires_spacy
def test_stack_A2_A3_provenance_single_source_and_pickle(tmp_path):
    import pickle
    cfg = _cfg(tmp_path)
    for ab in (FEATURE_STATE_ONLY_ABLATION, RELATIVE_FW_ONLY_ABLATION):
        est = make_factory_for_ablation("stylo_stack", cfg, ablation=ab)()
        ...
        assert rt.ablation_ == ab and "_axes" in rt.__dict__ and "ablation_" not in rt.__dict__
