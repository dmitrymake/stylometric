"""Frozen external A0/A4 outputs form a strict, read-only golden contract.

Captured from commit `f1b8e165` by `scripts/artifacts/capture_work_balanced_ablation_goldens.py`
before the estimator-axis routing refactor. This test only READS the fixture (never regenerates it, never fits an
...
def test_provenance_contract_and_authoritative_hashes():
    ...
    capture_tool = ROOT / "scripts" / "artifacts" / "capture_work_balanced_ablation_goldens.py"
    assert e["capture_tool_sha256"] == hashlib.sha256(capture_tool.read_bytes()).hexdigest()
