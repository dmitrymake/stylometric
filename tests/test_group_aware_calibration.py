"""Group-aware calibrator selection and work-balanced stack integration.
    ...
    def test_class_absent_from_validation_side_disables(self, monkeypatch):
        ...
        class _FakeSGKF:
            ...
            def split(self, X, yy, groups):
                # Structurally valid two-fold partition; each validation side
                # contains only one class, so the registered identity fallback
                # is reached only after the structural preflight passes.
                return [
