"""B4-B increment 2: heavy capture-environment replay gate for the frozen A0/A4 external goldens.
...
``B4_LIVE_GOLDEN_REPLAY=1``; once the flag is set it is fail-closed — it verifies the environment,
...
thread pools, then replays the WHOLE frozen contract.
    ...
    B4_LIVE_GOLDEN_REPLAY=1 PYTHONHASHSEED=0 OMP_NUM_THREADS=2 MKL_NUM_THREADS=2 \\
    OPENBLAS_NUM_THREADS=2 uv run pytest tests/test_b4_live_goldens.py -q
...
(a separate replay helper — the canonical ``scripts/capture_b4_goldens.py`` is never edited) and
...
# ── env gate FIRST, before numpy is imported (mirrors scripts/capture_b4_goldens.py) ──
import contextlib
import hashlib
import importlib
import os
...
if os.environ.get("B4_LIVE_GOLDEN_REPLAY") != "1":
    pytest.skip(
...
if _bad_env:
    raise RuntimeError(
        f"B4_LIVE_GOLDEN_REPLAY=1 requires the pinned deterministic env set BEFORE numpy import; "
...
# ── instrumentation mirroring scripts/capture_b4_goldens.py (never edits that frozen tool) ──
def _sha(b) -> str:
...
def cfg(tmp_path_factory):
    cache = tmp_path_factory.mktemp("b4_live_replay")
