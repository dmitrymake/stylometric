"""P0 scientific-reset guarantees: strict JSON, claim-status vocabulary, release hygiene.

These tests encode the Phase P0 gate:
...
# --------------------------------------------------------------------------- #
# every committed JSON artifact must be strict
# --------------------------------------------------------------------------- #
def _tracked_json_files() -> list[str]:
    out = subprocess.check_output(["git", "ls-files", "*.json"], cwd=REPO_ROOT, text=True)
...
def test_all_committed_json_is_strict():
    offenders = []
    ...
    assert not offenders, "non-strict committed JSON:\n" + "\n".join(offenders)
