"""B2a: stylo_stack work-balanced feature/loss wiring.

Feature and loss only — the stack stays BLOCKED in the run engines under work_balanced (calibration
is B3), so the work-balanced path is exercised here by direct construction. Channel tests are
spaCy-free (hashing channels); the loss tests monkeypatch ``make_channels`` to the two hashing
channels so the stack fits without spaCy.
"""
...
class TestStackLossWiring:
    ...
    def test_legacy_calls_strict_two_arg_channel(self, monkeypatch):
        # a genuine pre-B2a channel takes EXACTLY two positional args; the legacy stack must not pass
...
# ── hashing channel routes the B1 work-id validator ───────────────────────────
