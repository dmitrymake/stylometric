...
const FINDINGS = [
  ...
  },
  {
    ...
    text:
      ...
      "немногословного, Волошина. Такие авторы раздувают общую точность. Поэтому главный итог " +
      "мы считаем не по ней, а по средней точности на автора: там каждый весит одинаково, без " +
      "поблажки за объём. Её надёжные границы — это доверительный интервал.",
  },
  {
    severity: "info",
...
// Худший по узнаваемости автор публикуемого среза — динамически из данных (имя, число верных книг).
      ...
      <div className="wrap flow">
        ...
        <div className="reveal module">
          ...
          <p className="prose muted" style={{ marginBottom: 8 }}>
            Общая точность <span className="mono" style={{ color: "var(--success)" }}>{fmtScore(HEADLINE.accuracy)}</span> против
            точности по авторам <span className="mono" style={{ color: "var(--warning)" }}>{fmtScore(HEADLINE.styloMacroF1)}</span> (та же
            наша модель, среднее по всем авторам поровну, без поблажки многословным) — и разрыв не случаен.
            Общую точность держат авторы с большим объёмом текста. Провалы оседают на тех, у кого текста мало.
          ...
          {HEADLINE.trainingWeighting === "chunk_weighted_training_legacy" && (
            <p className="mono muted" style={{ fontSize: 12, marginBottom: 8 }}>
              ...
              ещё впереди — цифры могут немного сдвинуться.
            </p>
          )}
          <p className="prose muted" style={{ marginBottom: 8 }}>
            Каждая строка — автор и то, как часто модель верно его узнаёт при проверке по целым книгам:
          ...
          <p className="prose muted" style={{ marginBottom: 22 }}>
            Общая точность и средняя точность по авторам описывают весь
            корпус, а не пригодность метода к одному тексту. Для отдельного спорного текста решает не среднее число,
            а узкая панель: близкие авторы той же школы, эпохи и типа письма — и то, куда именно утекает текст.
            Поэтому один и тот же автор в разных панелях узнаётся по-разному — например, {worstPd.name}.{" "}
            В публикуемом срезе классиков — тех, кто умер больше 70 лет назад ({CORPUS.pd.authors}{" "}
            {plu(CORPUS.pd.authors, ["автор", "автора", "авторов"])}; Олеша,
            Катаев и Зощенко в него не входят), — хуже всех даётся именно он: модель узнаёт его в{" "}
            {worstPdCorrect} из {worstPd.books} книг. А в полном бенчмарке из {CORPUS.benchmark.authors} авторов, где
            рядом стоят нули, тот же автор узнаётся ещё реже — {recPhrase(worstPd.id)}. Сменится набор
            соседей — сменится и ближайший «двойник» у спорного текста, а с ним и ответ.
          </p>
        ...
        <p className="verdict reveal">
          <strong style={{ color: "var(--text)" }}>Это основа любого вывода.</strong> На перекос между авторами отвечает
          точность по авторам (macro-F1). На дубли и скрытые подсказки — валидатор и неделимая книга при проверке по целым книгам. Иначе любая цифра говорила бы о чистоте данных, а не о стиле.
        </p>
      </div>
    </section>
  );
