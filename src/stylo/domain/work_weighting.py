"""Evaluation-independent work-balanced training estimand primitives.
...
W (not 1) to keep C comparable to a work-level fit.

``chunk_weighted_legacy`` returns ``None`` (uniform sklearn weighting), reproducing the
legacy headline. These are the two supported training weightings, mirroring the case
...
class AblationNotImplementedError(NotImplementedError):
    """An ablation is requested for a model/cell combination without estimator wiring.

    The two production corners (all-off == legacy, all-on == work_balanced) map to runtime weighting
    enums. The single-axis A1/A2/A3 cells use dedicated audit estimators where the requested axis is
    meaningful. Combined off-diagonal configurations (WFR ``110, 101, 011``) have no estimator wiring
...
class AblationNotApplicableError(ValueError):
    ...
    The per-axis *effective* status recorded by an applicability-aware runner is a separate,
    spec-deterministic property and is not collapsed into ``reason``. Both char_cos and the Delta
    family use equal-work centroids, so their effective
...
# A1: work-balanced loss/calibration, legacy (A0) feature state. Audit-only.
WEIGHTS_ONLY_ABLATION = AblationConfig(True, False, False)
# A2/A3: feature-state-only and relative-FW-only. Audit-only, legacy A0 loss.
...
# runtime value maps to the frozen legacy headline claim.
_CLAIM_LABELS = {
...
def to_claim_label(training_weighting: str | None) -> str:
    ...
    ``chunk_weighted_legacy`` (runtime) -> ``chunk_weighted_training_legacy`` (legacy claim).
    """
