...
@dataclasses.dataclass(frozen=True)
class VerifiedInstalledPackage:
    """Verified wheel state with scientific and installer attestations split.

    ``package_payload_sha256`` identifies the canonical manifest of verified
    scientific payload members. ``record_bytes_sha256`` identifies the literal
    on-disk ``RECORD`` bytes for before/after integrity checks, while
    ``legacy_record_text_sha256`` reproduces the former newline-normalized
    diagnostic. Both are deliberately absent from :class:`ResolvedNLPIdentity`.
    """

    package_version: str
    package_payload_sha256: str
    record_bytes_sha256: str
    legacy_record_text_sha256: str
...
def verified_installed_package_payload(
    name: str,
) -> VerifiedInstalledPackage:
    """Verify an installed wheel and identify its canonical payload bytes.

    Every ``RECORD`` row remains authoritative and is checked before the
    installer-dependent rows are excluded from the scientific manifest.
    Generated bytecode is accepted only when it derives exactly from a verified
    source. The literal ``RECORD`` digest remains a separate temporal and
    diagnostic attestation, never scientific identity material.
    """

    distribution = importlib.metadata.distribution(name)
    root_path = pathlib.Path(distribution.locate_file(""))
    ...
    metadata_root_path = pathlib.Path(metadata_root)
    if metadata_root_path.is_symlink() or not metadata_root_path.is_dir():
        raise RuntimeError(
            f"installed spaCy model {name!r} has an unsafe metadata root"
        )
    own_record_path = metadata_root_path / "RECORD"
    ...
    try:
        record_bytes = own_record.read_bytes()
        record = record_bytes.decode("utf-8")
    except (OSError, UnicodeError) as exc:
        raise RuntimeError(
            f"installed spaCy model {name!r} has an invalid wheel RECORD"
        ) from exc
    if not record:
        raise RuntimeError(
    ...
    try:
        rows = csv.reader(io.StringIO(record, newline=""), strict=True)
    ...
    for member in sorted(set(parsed) - {own_record_member}):
        member_path, digest_spec, size_text = parsed[member]
        is_bytecode = (
            member_path.suffix == ".pyc"
            and member_path.parent.name == "__pycache__"
        )
        if not digest_spec and (size_text or not is_bytecode):
            raise RuntimeError(
    ...
    owned_roots.add(own_record.parent)

    allowed_unrecorded = set(bytecode_sources)

    def reject_walk_error(error: OSError) -> None:
        raise RuntimeError(
            f"installed spaCy model {name!r} package tree cannot be "
            f"inspected safely: {error.filename!r}"
        ) from error
    ...
    for member, (_member_path, digest_spec, size_text) in parsed.items():
        ...
        installer_metadata = (
            _member_path.parent == own_row[0].parent
            and _member_path.parts[1]
            in {"INSTALLER", "REQUESTED", "direct_url.json"}
        )
        generated_bytecode = member in bytecode_sources
        if not installer_metadata and not generated_bytecode:
    ...
    if verified == 0 or not payload_members:
        raise RuntimeError(
            f"installed spaCy model {name!r} RECORD has no verified payload"
        )
    payload_members.sort(key=lambda row: str(row["relative_path"]))
    ...
    return VerifiedInstalledPackage(
...
def _verified_model_package_binding(
    name: str,
    package_identity: VerifiedInstalledPackage,
    ...
    distribution = importlib.metadata.distribution(name)
    if type(package_identity) is not VerifiedInstalledPackage:
        raise RuntimeError(
            f"spaCy model {name!r} has an invalid verified package identity"
        )
    metadata_root = getattr(distribution, "_path", None)
    if metadata_root is None:
        raise RuntimeError(
    ...
    record_path = metadata_root_path / "RECORD"
    ...
    if (
        str(distribution.version) != package_identity.package_version
        or hashlib.sha256(record_bytes).hexdigest()
        != package_identity.record_bytes_sha256
        or hashlib.sha256(
            record.replace("\r\n", "\n").replace("\r", "\n").encode("utf-8")
        ).hexdigest()
        != package_identity.legacy_record_text_sha256
    ):
        raise RuntimeError(
...
def _verified_spacy_load(
    ...
    *,
    ...
    def reverify_bound(
        name: str,
        before: VerifiedInstalledPackage,
        before_binding: tuple[str, str, str, str, tuple[str, ...]],
        *,
        outcome: str,
    ) -> VerifiedInstalledPackage:
        try:
            after = verified_installed_package_payload(name)
    ...
    def verified_fallback():
        ...
        try:
            before = verified_installed_package_payload(fallback)
    ...
    try:
        before = verified_installed_package_payload(model)
    except importlib.metadata.PackageNotFoundError as exc:
...
def _installed_package_identity(name: str, nlp) -> tuple[str, str]:
    """Return installed version and a verified package payload/metadata digest.
    ...
    try:
        verified = verified_installed_package_payload(name)
        return (
            verified.package_version,
            verified.package_payload_sha256,
        )
...
def _build_nlp_identity(
    *,
    ...
    nlp,
    max_length: int,
    package_identity: VerifiedInstalledPackage | None = None,
    disabled_pipes: Sequence[str] = _DISABLE,
) -> ResolvedNLPIdentity:
    if type(package_identity) is VerifiedInstalledPackage:
        version = package_identity.package_version
        payload_sha = package_identity.package_payload_sha256
    elif package_identity is None:
        version, payload_sha = _installed_package_identity(resolved, nlp)
    else:
        raise RuntimeError("invalid verified NLP package identity")
    payload = {
        "requested_model": requested,
        "resolved_model": resolved,
        "fallback_used": resolved != requested,
        "package_version": version,
        "package_payload_sha256": payload_sha,
