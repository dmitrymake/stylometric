"""Model-layer calibration of decision scores; method selection by OOF-NLL.
...
def choose_calibrator(oof: np.ndarray, y: np.ndarray,
    ...
    ``groups`` заданы (work_balanced) — **group-aware**: StratifiedGroupKFold по работам, чтобы
    ...
    The returned selection losses are descriptive method-selection diagnostics,
    not unbiased evidence for a downstream meta-validation row. The legacy
    stack refits the chosen calibrator on all OOF labels before mode selection;
    that internal score is explicitly withdrawn until nested/cross-fitted
    group calibration replaces this compatibility path.
    """
